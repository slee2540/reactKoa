...
class WritePostContainer extends Component {
  ...
  handlePost = async () => {
    ...
    if (value.length < 5) {
      ...
      return toast(newMessage('너무 짧습니다. 5자 이상 입력하세요.'), { type: 'error' });
    }

    if (value.length > 1000) {
      HomeActions.changeWritePostInput('');
      return toast(newMessage('최대 1000자까지 입력 할 수 있습니다.'), { type: 'error' });
    }

    try {
      await HomeActions.writePost(value);
      return toast(newMessage('생각이 작성되었습니다.'), { type: 'success' });
    } catch (e) {
      return toast(newMessage('오류가 발생했습니다.'), { type: 'error' });
    }
  };
...
export default connect(
  state => ({
    value: state.home.writePost.value
  }),
  dispatch => ({

...
class CommentBlockContainer extends Component {
  ...
  comment = () => {
    ...
    if (status.value === '') return; // 텍스트가 비어있으면 아무 작업도 하지 않습니다
  ...
  render() {
    ...
    const { visible, value } = status || {}; // status 가 존재하지 않는 경우를 위한 예외 케이스
    const { handleChange, handleKeyPress } = this;
    // console.log(status);
    if (!visible) return null; // visible 이 false 면 아무것도 렌더링하지 않기
    return <CommentBlock value={value} onChange={handleChange} onKeyPress={handleKeyPress} comments={post.comments} />;
  }
}
...
export default connect(
  (state, ownProps) => ({
    ...
    status: state.posts.comments[ownProps.post._id]
    // test: ownProps.post._id
  }),
  dispatch => ({
